...
-- 1. Tighten orders INSERT policy for authenticated customers
...
-- 2. Harden create_guest_order: hardcode payment_status to 'unpaid'
...
BEGIN
  ...
  INSERT INTO public.orders (
    ...
    payment_approved, payment_approved_by, payment_approved_at
  )
  VALUES (
    'online'::sale_channel,
    'pending'::order_status,
    'unpaid'::payment_status,  -- HARDCODED: never trust client
...
-- 3. Restrict public visibility of product_variants.stock_quantity via column-level grants.
--    Full table SELECT grant is revoked; column-level SELECT is granted for all
--    non-sensitive columns. Admin/staff/stock_manager use has_role RLS + separate
--    admin RPCs to read stock levels.
REVOKE SELECT ON public.product_variants FROM anon, authenticated;

GRANT SELECT (
  id, product_id, variant_name, sku, barcode, price_override,
  attributes, is_active, created_at, updated_at, image_url, description
) ON public.product_variants TO anon, authenticated;

-- Keep write privileges intact for admin/staff/stock_manager (still filtered by RLS)
GRANT INSERT, UPDATE, DELETE ON public.product_variants TO authenticated;
GRANT ALL ON public.product_variants TO service_role;

-- 4. Allow authenticated buyers/recipients to view their own gift vouchers
CREATE POLICY "Buyers and recipients can view own vouchers"
...
USING (
  ...
  AND (
    lower(buyer_email) = lower((SELECT auth.jwt() ->> 'email'))
