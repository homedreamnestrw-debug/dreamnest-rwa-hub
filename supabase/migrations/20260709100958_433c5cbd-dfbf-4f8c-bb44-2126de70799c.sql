...
ALTER TABLE public.business_settings ADD COLUMN IF NOT EXISTS vouchers_enabled boolean NOT NULL DEFAULT true;
...
CREATE OR REPLACE FUNCTION public.get_public_business_settings()
 RETURNS TABLE(
   ...
   receipt_logo_url text, shop_enabled boolean, vouchers_enabled boolean
 )
 LANGUAGE sql
...
AS $$
  SELECT
    ...
    receipt_header, receipt_footer, receipt_logo_url, shop_enabled, vouchers_enabled
