...
export default function POS() {
  ...
  const { data: products } = useQuery({
    ...
    refetchInterval: 30000, // refresh stock every 30s
  });
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Create order items (triggers stock deduction)
      ...
      toast.success(`Sale #${order.order_number} completed!`);
      setCart([]);
      setCustomerNote("");
    } catch (err: any) {
    ...
    <>
                ...
                <div>
                  ...
                  >
                    {paymentMethods.map((pm) => (
                      <label
                        key={pm.value}
                        className={`flex items-center gap-2 p-2.5 rounded-md border cursor-pointer text-sm transition-colors ${
          ...
          {receiptOrder && (
            <div id="receipt" className="space-y-4">
              <div className="text-center border-b pb-4">
                ...
                {settings?.address && (
                  <p className="text-xs text-muted-foreground">{settings.address}, {settings?.city}</p>
                )}
                {settings?.phone && (
              ...
              <div className="text-sm space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{receiptOrder.order_number}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Date</span>
                  <span>{format(new Date(receiptOrder.created_at), "MMM d, yyyy h:mm a")}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Payment</span>
                  <span className="capitalize">{receiptOrder.payment_method.replace("_", " ")}</span>
                </div>
              </div>
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between font-medium text-base pt-1">
                  <span>Total</span>
                  <span className="font-serif">{formatPrice(receiptOrder.total)}</span>
                </div>
              </div>
      ...
      {receiptOrder && (
        <div className="hidden print:block p-4 max-w-[300px] mx-auto text-sm">
          ...
          <p>Payment: {receiptOrder.payment_method.replace("_", " ")}</p>
          <hr className="my-2" />
          {receiptOrder.items.map((item) => (
            <div key={item.product_id} className="flex justify-between">
